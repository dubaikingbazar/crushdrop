...
function FormScreen({ form, setForm, setScreen, onSend, sending, sendError }) {
  return (
    <div className="card">
      ...
      <p className="sub" style={{textAlign:"left",marginBottom:22}}>Sending is always free. Your name stays hidden unless they pay to reveal.</p>

      <label className="label">Your name (for reveal only)</label>
      <input
        className="input"
        placeholder="e.g. Rahul — only shown if they pay"
...
export default function CrushDrop() {
  ...
  const [form, setForm]           = useState({ senderName: "", name: "", contact: "", message: "", reveal: true, isPremium: false });
  const [txnId, setTxnId]         = useState("");
  const [revealTxnId, setRevealTxnId] = useState("");
  ...
  useEffect(() => {
    ...
    if (msgId) {
      ...
      supabaseFetchById(msgId)
        .then(msg => { setReceiverMsg(msg); setReceiverLoading(false); })
        .catch(() => setReceiverLoading(false));
    }
  }, []);
  ...
  // Reveal flow — verify txn then show sender name
  const handleReveal = async () => {
    if (revealTxnId.length < 5) return;
    setRevealLoading(true);
    try {
      await supabaseReveal(currentMsgId || "demo");
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .app {
      ...
      font-family:'DM Sans',sans-serif; color:#3D2B35;
    ...
    .card {
      ...
      border-radius:24px; padding: 28px 20px 32px;
    ...
    .input {
      width:100%; background:rgba(253,232,238,0.4);
    ...
    .sender-name-hint {
      ...
      border:1px dashed rgba(232,114,138,0.25);
      border-radius:12px; padding:10px 14px;
      font-size:12px; color:#B08898; margin-bottom:16px;
      line-height:1.5;
    }
    ...
    .pill-switch {
      ...
      border-radius:100px; position:relative; flex-shrink:0; transition:background 0.25s;
    }
    ...
    .msg-card {
      ...
      border-radius:20px; padding:20px 18px; margin:16px 0; position:relative;
    }
    ...
    .blur-cta {
      ...
      box-shadow:0 4px 14px rgba(232,114,138,0.4); min-height: 44px;
    }
    .blur-sub { font-size:11px; color:rgba(60,30,40,0.6); }

    .countdown-box {
      background:rgba(232,114,138,0.07); border:1.5px solid rgba(232,114,138,0.2);
    ...
    .countdown-timer {
      ...
      animation:timerPulse 2s ease-in-out infinite; letter-spacing:1px;
    }

    .more-crushes-box {
      background:linear-gradient(135deg,rgba(232,114,138,0.08),rgba(201,160,180,0.1));
      border:1.5px solid rgba(232,114,138,0.2);
      border-radius:16px; padding:16px; margin-top:12px; cursor:pointer;
    ...
    .link-box {
      ...
      word-break:break-all; margin-bottom:10px; text-align:center; font-weight:500;
    }
    ...
    .success-circle {
      ...
      justify-content:center; font-size:32px; margin:0 auto 18px;
    ...
    .demo-chip {
      background:rgba(201,160,180,0.12); border:1px solid rgba(201,160,180,0.2);
      border-radius:100px; padding:7px 16px; font-size:12px; color:#C9A0B4;
      cursor:pointer; font-family:'DM Sans',sans-serif;
    ...
    .premium-badge {
      ...
      border:1px solid rgba(255,165,0,0.3); border-radius:100px; padding:3px 10px;
      font-size:11px; font-weight:600; color:#B8860B; margin-bottom:10px;
    }
    ...
    <div className="card">
      ...
      <div style={{marginBottom:22}}>
        {[
          ...
          ["🔐","Reveal when you're ready","They pay ₹49 to find out who sent this"],
  ...
  const Payment = () => (
    <div className="card">
      ...
      <input className="input" placeholder="e.g. 427891234567" value={txnId} onChange={e=>setTxnId(e.target.value)} inputMode="numeric"/>
      {sendError && <p style={{color:"#E8728A",fontSize:13,marginBottom:10,textAlign:"center"}}>{sendError}</p>}
      <button className="btn btn-premium" onClick={()=>{ if(txnId.length>4) handleSend(txnId); }} disabled={sending} style={{opacity: sending?0.7:1}}>
  ...
  const Sent = () => (
    <div className="card" style={{textAlign:"center"}}>
      ...
      <button className="btn btn-ghost" onClick={()=>{
        setForm({senderName:"",name:"",contact:"",message:"",reveal:true,isPremium:false});
        setTxnId(""); setSentMsgId(null); setScreen(SCREENS.LANDING);
      }}>Send another 💗</button>
    </div>
  );
  ...
  const RevealPayment = () => (
    <div className="card">
      <button className="back-btn" onClick={()=>setScreen(SCREENS.RECEIVER)}>← Back</button>
      <div style={{textAlign:"center",marginBottom:16}}>
        <div style={{fontSize:48}}>🔓</div>
      </div>
      <h2>Reveal your admirer 💗</h2>
      <p className="sub" style={{marginBottom:18}}>Pay ₹49 to find out who secretly likes you. Completely private.</p>
      <div className="receipt">
        <div className="receipt-row"><span className="receipt-label">Name reveal</span><span className="receipt-val">₹49</span></div>
        <div className="receipt-row"><span className="receipt-label">Total</span><span className="receipt-val receipt-total">₹49</span></div>
      </div>
      <div className="upi-box">
        <div style={{fontSize:12,color:"#C9A0B4"}}>Pay via UPI</div>
      ...
      <label className="label">Paste your UPI transaction ID</label>
      <input className="input" placeholder="e.g. 427891234567" value={revealTxnId} onChange={e=>setRevealTxnId(e.target.value)} inputMode="numeric"/>
      <button
        className="btn btn-mauve"
        onClick={handleReveal}
        disabled={revealLoading || revealTxnId.length < 5}
        style={{opacity: revealLoading ? 0.7 : 1}}
      >
        {revealLoading ? "Revealing... 💫" : "Reveal their name 🔓"}
      </button>
      <p style={{textAlign:"center",fontSize:11,color:"#C9A0B4",marginTop:10}}>Completely private · only you can see this</p>
    </div>
  );
  ...
  const Receiver = () => {
    ...
    const isRevealed = msg?.is_revealed || revealed;
      ...
      <div className="card">
        ...
        <div className="msg-card">
          ...
          <div style={{position:"relative"}}>
            <div className={`msg-text ${messageBlurred?"blurred":""}`} onClick={()=>messageBlurred&&setMessageBlurred(false)}>
              "{displayMessage}"
            </div>
            {messageBlurred && (
        ...
        {isRevealed ? (
          <div className="reveal-result">
            ...
            {!senderName && (
              <div style={{fontSize:13,color:"#B08898",marginBottom:12}}>They chose to stay anonymous 🤫</div>
            )}
            <button className="btn btn-primary" style={{animation:"none",boxShadow:"none",marginBottom:10,marginTop:16}} onClick={()=>goShare("receiver")}>
              📸 Share this moment!
            </button>
          ...
          <>
            {revealEnabled && (
              <>
                ...
                <button className="btn btn-mauve" onClick={()=>setScreen(SCREENS.REVEAL_PAYMENT)}>
    ...
    return (
      <div className="card">
        ...
        <button className="native-share-btn" onClick={handleNativeShare}>
          {shareStatus==="generating" ? <><div className="spinner"/> Generating story...</>
  ...
  const render = () => {
    switch(screen) {
      ...
      case SCREENS.REVEAL_PAYMENT: return <RevealPayment/>;
